...
namespace LeagueSharp.Common
{
    ...
    public class Spell
    {
        ...
        public Vector3 From
        {
            get { return !_from.LSTo2D().LSIsValid() ? ObjectManager.Player.ServerPosition : _from; }
            set { _from = value; }
        }
        ...
        {
            get { return !_rangeCheckFrom.LSTo2D().LSIsValid() ? ObjectManager.Player.ServerPosition : _rangeCheckFrom; }
            ...
            int minTargets = -1)
        {
            ...
            {
                //Target out of range
                if (RangeCheckFrom.LSDistance(unit.ServerPosition, true) > GetRangeSqr(unit))
                {
        ...
        {
            if (!Slot.LSIsReady() || From.LSDistance(unit.ServerPosition, true) > GetRangeSqr(unit))
            {
                return false;
            }
        ...
        public bool Cast(Vector3 fromPosition, Vector3 toPosition)
        {
            ...
            return Slot.LSIsReady() && ObjectManager.Player.Spellbook.CastSpell(Slot, fromPosition, toPosition);
        }
        ...
        {
            return Slot.LSIsReady() && unit.LSIsValidTarget(Range);
        }
