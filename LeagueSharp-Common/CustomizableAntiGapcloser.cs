...
namespace LeagueSharp.Common
{
    ...
    public static class CustomizableAntiGapcloser
    {
        ...
        public static void AddToMenu(Menu mainmenu)
        {
            ...
            foreach (var gapclose in Spells.Where(x => ObjectManager.Get<AIHeroClient>().Any(y => y.ChampionName == x.ChampionName && y.IsEnemy)))
            {
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            foreach (
                var cGapcloser in
                    CActiveCGapclosers.Where(cGapcloser => cGapcloser.Sender.LSIsValidTarget())
                        .Where(
                            cGapcloser =>
